...
const FilterSelectfield = <T extends FieldValues>({ nameField, data, setValue, watch }: FilterSelectfieldProps<T>) => {
    ...
    return (
        <div className="border border-zinc-300 rounded-[0.45rem] overflow-hidden">
            <div className="flex flex-row items-center justify-center">{data.map((item, i) => (
                <div key={i} className={`w-[50px] relative font-semibold ${item === data.length ? "border-0" : "border-r"} border-zinc-300 ${item === selectedRadioBtn ? "bg-teal-700 text-white" : "text-zinc-700 hover:bg-zinc-300"} transition`}>
