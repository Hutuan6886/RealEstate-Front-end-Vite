...
import { FaFacebookF, FaInstagram, FaGithub, FaArrowRightLong } from "react-icons/fa6";
import { useEffect, useState } from "react";

const tableData = [
    { title: "For Professionals", data: [{ title: "Popular Cities", href: "/popular-cities" }, { title: "Houses for Rent", href: "/popular-cities" }, { title: "Houses for Buy", href: "/popular-cities" }] },
    { title: "For Professionals", data: [{ title: "Houses for Rent", href: "/search?formType=Rent" }, { title: "Houses for Buy", href: "/search?formType=Sell" }] },
    {
        title: "Explore HuuTuan", data: [
            { title: <div className="flex flex-row gap-2 items-center justify-start"><FaFacebookF size={13} /> facebook</div>, href: `${import.meta.env.VITE_MY_FACEBOOK}` },
            { title: <div className="flex flex-row gap-2 items-center justify-start"><FaInstagram size={13} /> instagram</div>, href: `${import.meta.env.VITE_MY_INSTAGRAM}` },
            { title: <div className="flex flex-row gap-2 items-center justify-start"><FaGithub size={13} /> github</div>, href: `${import.meta.env.VITE_MY_GITHUB}` }]
    },
    { title: "For Professionals", data: [{ title: "Popular Cities", href: "/popular-cities" }] },
]
const brands: string[] = ['Zillow', 'StreetEasy', 'HotPads', 'Out East', 'Privacy Portal', 'Cookie Preference', 'Do Not Sell or Share My Personal Information']
const corporate = [
    { name: "About Zillow Group", href: "#" },
    { name: "Fair Housing Guide", href: "#" },
    { name: "Careers", href: "#" },
    { name: "Investor Relations", href: "#" },
    { name: "Newsroom", href: "#" },
    { name: "Advertising Terms", href: "#" },
    { name: "Privacy", href: "#" },
    { name: "Terms of Use", href: "#" },
    { name: "Listings Quality Policy", href: "#" },
    { name: "Help", href: "#" },
    { name: "Privacy Portal", href: "#" },
    { name: "Cookie Preference", href: "#" },
...
const Footer = () => {
    ...
    return (
        <footer className="w-full lg:w-[70%] my-10 mx-auto">
            <div className="flex flex-col gap-10">
                {pathname === "/"
                    ? <div className={`flex flex-col md:grid grid-cols-${tableData.length} gap-8`}>
                        {tableData.map((tableCol, i) => (
                            <div key={i} className={`col-span-1 flex flex-col gap-1 text-zinc-700`}>
                                <h3 className="font-semibold mb-2 text-nowrap">{tableCol.title}</h3>
                                {tableCol.data.map((data, i: number) => (
                                    <Link key={i} target="_blank" to={data.href} className="text-xs hover:text-teal-700">{data.title}</Link>
                                ))}
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex flex-col gap-3 text-xs text-zinc-600">
                    <div className="flex flex-row flex-wrap  gap-1">Zillow Group ® Other Brands: {brands.map((name: string, i: number) => (
                        i === brands.length - 1 ? <p key={i}>• <Link to={"#"} className="text-teal-700 font-semibold hover:text-zinc-600 hover:underline">{name}<FaArrowRightLong className="inline-block ml-1" /> </Link></p> : <p key={i}>• <Link to={"#"} className="inline hover:text-teal-700 hover:underline">{name}</Link></p>
                    ))}</div>
                    <div className="flex flex-row flex-wrap gap-1">Trulia Corporate: {corporate.map((item, i: number) => (
                        i === corporate.length - 1 ? <p key={i}>• <Link to={item.href} className=" text-teal-700 font-semibold hover:text-zinc-600 hover:underline">{item.name}<FaArrowRightLong className="inline-block ml-1" /> </Link></p> : <p key={i}>• <Link to={item.href} className=" hover:text-teal-700 hover:underline">{item.name}</Link></p>
                    ))}</div>
                    <p>Zillow Group is committed to ensuring digital accessibility for individuals with disabilities.  We are continuously working to improve the accessibility of our web experience for everyone,  and we welcome feedback and accommodation requests. If you wish to report an issue or seek an  accommodation, please <Link to={"#"} className="text-teal-700 font-semibold hover:text-zinc-600 hover:underline">let us know. </Link>
                    </p>
                    <p className="text-[0.65rem]">Copyright © 2024 Trulia, LLC. All rights reserved. <Link to={"#"} className="text-teal-700 font-semibold hover:text-zinc-600 hover:underline">Equal Housing Opportunity.</Link> Have a Question? Visit our <Link to={"#"} className="text-teal-700 font-semibold hover:text-zinc-600 hover:underline">Help Center</Link> to find the answer.</p>
                </div>
                <svg width="70%" height="100%" viewBox="0 0 704 113" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="m-auto vertical-align:bottom">
